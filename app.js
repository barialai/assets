...
function validateState(input) {
  if(!input||![1,2].includes(input.version)||!Array.isArray(input.assets)||!Array.isArray(input.entries)||!Array.isArray(input.snapshots)||!input.settings||!input.profile) throw Error('This is not a supported Vault backup (version 1 or 2).');
  ...
  out.profile={name:text(input.profile.name||'My portfolio',40),subtitle:text(input.profile.subtitle||'Personal portfolio',80),photo:safeImage(input.profile.photo),badge:Boolean(input.profile.badge)};
...
function overview(){
  ...
  <section class="wealth-strip" aria-label="Cash and loan balances"><div class="wealth-item net"><span>Net worth</span><strong>${money(t.netWorth,{currency:'USDT'})}<small>USDT</small></strong><p>Assets + money owed to you &minus; loans you owe</p></div><div class="wealth-item"><span>Everyday cash</span><strong>${money(t.cash)}</strong><button class="text-link" data-page="cash">Cash &amp; bank accounts ${icon('arrow-right')}</button></div><div class="wealth-item"><span>Owed to you</span><strong>${money(t.owedToYou)}</strong><button class="text-link" data-action="loans-filter" data-filter="lent">Money you lent ${icon('arrow-right')}</button></div><div class="wealth-item"><span>You owe</span><strong>${money(t.youOwe)}</strong><button class="text-link" data-action="loans-filter" data-filter="borrowed">Loans to repay ${icon('arrow-right')}</button></div></section>
...
function exportLoans(){
  ...
  downloadBlob('vault-loans-'+today()+'.csv','\ufeff'+rows.map(r=>r.map(csvCell).join(',')).join('\r\n'),'text/csv');toast('Loan and repayment CSV prepared.');
}
