...
'use strict';
/* Assets v5. Local-first ledger with an optional authenticated cloud backend. */
(() => {
...
function drawPortfolioChart(){const host=$('#portfolio-chart');if(!host)return;const minDate=ui.period==='ALL'?'0000-00-00':shiftDate(-(Number.parseInt(ui.period)-1));let data=state.snapshots.filter(p=>p.date>=minDate&&p.date<=today());if(!data.length){host.innerHTML='<div class="range-empty">No snapshots in this period yet.</div>';return;}
  ...
  host.innerHTML=`<svg viewBox="0 0 ${W} ${H}" preserveAspectRatio="none" role="img" aria-label="Portfolio valuation history in USDT. ${data.length} saved daily snapshots."><defs><linearGradient id="portfolio-fill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#0030CF" stop-opacity=".28"/><stop offset="100%" stop-color="#0030CF" stop-opacity="0"/></linearGradient></defs>${grid}<path d="${area}" fill="url(#portfolio-fill)"/><path d="${path}" fill="none" stroke="#0030CF" stroke-width="2.3" vector-effect="non-scaling-stroke" stroke-linejoin="round" stroke-linecap="round"/><circle cx="${last.x}" cy="${last.y}" r="5" fill="#0030CF" opacity=".2"/><circle cx="${last.x}" cy="${last.y}" r="2.6" fill="#0030CF"/>${labels.join('')}<line id="chart-hover-line" x1="0" y1="${T}" x2="0" y2="${T+ih}" stroke="#949494" stroke-width=".7" stroke-dasharray="3 3" visibility="hidden"/></svg><div class="chart-tooltip" id="chart-tooltip"></div>`;
