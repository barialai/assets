...
const CACHE='vault-public-shell-v4-20260920';
...
self.addEventListener('activate',event=>event.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith('vault-public-shell-')&&k!==CACHE).map(k=>caches.delete(k)))).then(()=>self.clients.claim())));
self.addEventListener('fetch',event=>{
  ...
  event.respondWith(fetch(event.request).then(response=>{
    ...
    return response;
  }).catch(async()=>{const hit=await caches.match(cacheKey);return hit||new Response('Open Vault online once to install the offline interface.',{status:503,headers:{'Content-Type':'text/plain'}});}));
});
